...
use crate::artifact_contract::{ArtifactContract, EffortLevel, RequiredArtifact};
...
impl LargePromptClassifier {
            ...
            match Self::extract_contract(prompt, repo_path) {
                ...
                None => {
                    ...
                    PromptClassification::Ambiguous {
                        reason: "High project indicators but unclear artifact requirements"
                            .to_string(),
                    }
                }
            }
    ...
    fn score_prompt(prompt: &str) -> u8 {
        ...
        score += file_score.min(30);

        // Check for strictness indicators (+10 each, max 20)
        let mut strict_score = 0;
        let strict_indicators = [
            "do not hallucinate",
            "base everything strictly",
            "strictly on repo",
    ...
    fn extract_contract(prompt: &str, repo_path: Option<&Path>) -> Option<ArtifactContract> {
        ...
        let root = repo_path.unwrap_or(Path::new("."));
        Some(ArtifactContract::from_artifacts(
            root,
            artifacts,
    ...
    fn infer_artifact_type(filename: &str) -> ArtifactType {
        ...
        match ext.as_str() {
            ...
            "c" | "h" => ArtifactType::Code {
                language: "c".to_string(),
            },
            "cpp" | "hpp" | "cc" => ArtifactType::Code {
...
pub fn contract_to_chain(
    ...
    let steps: Vec<PersistentChainStep> = contract
        .artifacts
        .iter()
        .map(|artifact| PersistentChainStep {
...
impl SimpleCommandParser {
    ...
    pub fn parse_simple(input: &str, repo_path: impl AsRef<Path>) -> Option<ArtifactContract> {
        ...
        if normalized.starts_with("create ") {
            ...
            if !files.is_empty() {
                let artifacts: Vec<RequiredArtifact> = files
                    .iter()
                    .map(|f| RequiredArtifact {
                        path: root.join(f),
        ...
        if normalized.contains("test") && normalized.contains("for") {
            ...
            let test_artifacts = vec![RequiredArtifact {
