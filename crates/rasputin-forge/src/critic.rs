...
use crate::types::{
    CodeEntity, EntityKind, Flaw, FlawCategory, FlawQueue, ForgeConfig, ForgeError,
};
...
impl LocalCritic {
            ...
            if lines > 50 {
                let flaw = self
                    .create_flaw(
                        ...
                        format!(
                            "Function '{}' spans {} lines. Consider extracting helper functions.",
                            entity.name, lines
                        ),
                    )
                    .await?;
            ...
            if entity.content.contains("TODO") || entity.content.contains("FIXME") {
                let flaw = self
                    .create_flaw(
                        ...
                        format!(
                            "Entity '{}' contains unresolved TODO/FIXME comments.",
                            entity.name
                        ),
                    )
                    .await?;
    ...
    async fn create_flaw(
        ...
        let content = tokio::fs::read_to_string(&entity.file_path)
            .await
            .map_err(|e| ForgeError::Io(e))?;
        ...
        Ok(Flaw {
            ...
            file_path: entity
                .file_path
                .strip_prefix(&self.config.target_repo)
    ...
    async fn run_linters(&self) -> Result<Vec<Flaw>, ForgeError> {
        ...
        for linter_cmd in &self.config.linters {
            ...
            let linter_flaws = self
                .parse_linter_output(&format!("{}{}", stdout, stderr), linter_cmd)
                .await?;
