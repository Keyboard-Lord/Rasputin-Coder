...
use crate::types::{Flaw, ForgeConfig, ForgeError, ForgeStats, LoopStatus, PatchResult};
use hex;
...
impl ExhaustionLoop {
    ...
    pub async fn run(&mut self) -> Result<ForgeStats, ForgeError> {
        ...
        while self.status == LoopStatus::Running {
            ...
            if self.iteration > self.config.max_iterations {
                ...
                warn!(
                    "[FORGE] Loop stalled after {} iterations",
    ...
    async fn iteration(&mut self) -> Result<bool, ForgeError> {
        ...
        while let Some(flaw) = flaw_queue.pop_next() {
            println!(
                "\n[FLAW] {:?}:{} - {:?}",
                flaw.file_path, flaw.line, flaw.category
            );
    ...
    async fn process_flaw(&mut self, flaw: &Flaw) -> Result<bool, ForgeError> {
        ...
        if current_hash != flaw.content_hash {
            warn!(
                "[FORGE] File {:?} has changed since flaw detection",
                flaw.file_path
            );
            // Still try to fix it
        }
        ...
        loop {
            ...
            if !all_applied {
                ...
                println!(
                    "       [SELF-HEALING] Attempt {}/{} with compiler feedback",
            ...
            match self.run_tests().await {
                ...
                Ok(false) => {
                    ...
                    let healed_response = self.ollama.generate_fix(flaw, &healing_prompt).await?;
                    let healed_patches =
                        self.chisel.parse_patches(&healed_response, &flaw.file_path);
                    ...
                    println!(
                        "       [SELF-HEALING] Attempt {}/{} with test feedback",
                        retry_count, max_retries
                    );
                }
                Err(e) => {
